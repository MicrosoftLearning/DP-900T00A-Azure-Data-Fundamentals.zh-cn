...
`size` 必须小于或等于 `require('buffer').kMaxLength` 的值（在 64 位架构上，`kMaxLength` 为 `(2^31)-1`）。 否则会引发 [`RangeError`][]。 如果指定 `size` 小于或等于 0，那么将会创建零长度缓冲区。
...
但如果使用 `Number` 参数调用 `toHex`，会发生什么情况？

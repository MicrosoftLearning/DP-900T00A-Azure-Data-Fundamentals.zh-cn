...
1. 在 Azure 门户主页的左上角，选择“**&#65291; 创建资源**”，并搜索 `Storage account`。 然后在出现的“存储帐户”页中，选择“创建”。
1. 在“创建存储帐户”页上，输入以下值：
    - **订阅**：选择 Azure 订阅。
    - 资源组：使用所选名称创建新资源组。
...
1. 在“**容器**”页中，选择“**&#65291; 容器**”，然后添加一个名为 `data` 的新容器，其匿名访问级别为“**专用（不允许匿名访问）**”。
...
1. 从 `https://aka.ms/product2.json` 下载 [product2.json](https://aka.ms/product2.json?azure-portal=true) JSON 文件，并将其保存在计算机上（之前下载 **product1.json** 的同一文件夹中），稍后将其上传到 Blob 存储。
